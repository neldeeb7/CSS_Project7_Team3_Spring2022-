...
# KSH: I have removed the view command to show the results in te markdown file instead and included na.rm=T when we find min max (So we do not get NAs in the data frame)
```
...
We choose to investigate the state of Louisiana. Since the time measure of the data set is years, we need to convert the date of adoption to years in th column `Year_Adopted`. This essentially means that we assume that a state is treated from the 1. of January the year it adopted the Medicaid expansion.
...
The synthetic control can meet the parallel trend assumptions more precisely.
...
It does create an interpretation problem if the graph/visual representation of weights does not display negative weights. Ultimately, I think that as long as the error terms are small and there is a match pre-treatment, it is not a problem. 
...
We run the model that treats each state individually. This is done by setting `nu` equal to zero. `nu` determines how much pooling to do. A value of 0 will fit a separate synthetic control for each model, whereas a value of 1 will pool all units together. Leaving this argument blank will have `multisynth` search for the best value of `nu` that minimizes L2 loss. We also `n_leads`= 5, which determines how long past treatment effects should be estimated for. For some states there are fewer than 5 post treatment periods and for these states, it is just not possible to calculate the difference between synthetic control and treatment in all 5 periods. 
...
We can see that the further in time an observation is the bigger is the effect of the treatment. At least that is the general picture. The l2 imbalance is reported to be ??????? which means ???????
...
When using the time cohorts to pool together units, we pool together states that adopted Medicaid expansion within the same year, instead of based on ????????????????. 
...
Now the average ATT estimate across all states and periods is -0.014, which is very similar to what we found above. The adoption of Medicare expansion on average decreased the uninsured rate by 1.4 percent points. The L2 imbalance is reported to be ????? which is ??????. Again a pattern of an increasing treatment effect appears, where higher effects are seen the further away the observation is from time of treatment. 
...
Like when we where comparing effects at the state level, there also seems to be heterogenous effects depending on the time of treatment. The effect of the treatment seems to be much larger on states which implemented it early compared to states that implemented it late. 
...
- **Answer**: Fuzzy RD can accommodate situations where there is potentiall selection bias, and strong RD assumes no selection bias. 
