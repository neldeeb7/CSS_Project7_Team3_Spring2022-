...
- **Answer**: 
It does create an interpretation problem if the graph/visual representation of weights does not display negative weights. The weights are no longer a measure of similarity. Ultimately, I think that as long as the error terms are small and there is a match pre-treatment, it is not a problem. 
...
- **Answer**: Both DiD and synthetic control enable us to compare generalizable trends more than individual-level data units. If DiD was estimated at the individual level there would be no external validity. Think of the example, where we would investigate the effect on alcohol consumption from some policy intervention. Our research would be much more generalizeable if look at schools, cities, municipality, states, etc. than if we just compared one persen to another before and after. 
...
NE ADDED: 
RD has limitations with its external validity because its hard to generalize units far from the cutoff point while DiD/synthetic control have better external validity because they accounts for parallel and time-varying trends. 

RD is more useful to use when comparing groups that differ only in treatment while DiD compares groups that differ in both time and other variables assuming that the difference in outcomes is constant under the period of study. 
